...
if not os.getenv("TWITTER_API_KEY"):
    raise ValueError("TWITTER_API_KEY is not loading. Check your .env file and path.")
...
if not all([
    ...
    raise ValueError("Missing one or more Twitter OAuth1.0a credentials.")
...
def post_tweet(content: str, in_reply_to_id: str = None):
    """
    Post a tweet to Twitter using tweepy.API (OAuth1.0a).
    ...
    for attempt in range(max_retries):
        try:
            if in_reply_to_id:
                response = twitter_client.update_status(
                    status=content,
                    in_reply_to_status_id=in_reply_to_id,
                    auto_populate_reply_metadata=True
                )
            else:
                response = twitter_client.update_status(status=content)

            logger.info(f"Tweet posted successfully: {response.id_str}")
            return {
            ...
            logger.error(f"Twitter API error on attempt {attempt+1}: {str(e)}")
...
if __name__ == "__main__":
    logger.info("Twitter Posting Agent (Simple) started")
    try:
        asyncio.run(main())
