...
def get_mentions_and_replies(limit: int = 10, since_hours: int = 24):
    ...
    """
    try:
        log_to_database("info", f"Fetching mentions and replies (limit: {limit}, since_hours: {since_hours})")
        ...
        if not accounts:
            log_to_database("warning", "No accounts found to monitor")
            return {
        ...
        for username in accounts:
            try:
                ...
                log_to_database("error", f"Twitter API error for {username}: {str(e)}")
                continue
        
        log_to_database("info", f"Retrieved {len(mentions)} mentions and replies")
        return {
            "result": mentions,
            "count": len(mentions)
        }
        ...
        log_to_database("error", f"Error getting mentions and replies: {str(e)}")
        return {
...
def search_knowledge_for_reply(query: str, limit: int = 5):
    ...
    """
    try:
        log_to_database("info", f"Searching knowledge for reply: {query[:50]}..." + (f" (limit: {limit})" if limit != 5 else ""))
        ...
        log_to_database("info", f"Found {len(results)} relevant knowledge items for reply")
        return {
            "result": results,
            "count": len(results)
        }
        ...
        log_to_database("error", f"Error searching knowledge: {str(e)}")
        return {
...
def generate_and_post_reply(tweet_id: str, tweet_text: str, author_username: str):
    ...
    """
    try:
        log_to_database("info", f"Generating reply to tweet from @{author_username}", {"tweet_id": tweet_id})
        ...
        if post_response.data:
            ...
            log_to_database("info", f"Successfully posted reply to tweet {tweet_id}", {"reply_tweet_id": post_response.data.get("id")})
            ...
            log_to_database("error", "Failed to post reply to tweet")
            return {
        ...
        log_to_database("error", f"Error generating and posting reply: {str(e)}")
        return {
...
if __name__ == "__main__":
    ...
    log_to_database("info", "X Reply Agent started")
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        logger.info("X Reply Agent stopped by user")
        log_to_database("info", "X Reply Agent stopped by user")
    except Exception as e:
        ...
        log_to_database("info", "X Reply Agent stopped")
