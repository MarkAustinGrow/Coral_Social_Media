...
async def main():
    """Main agent execution loop with optimized OpenAI API usage"""
    ...
    logger.info(f"Starting X Reply Agent (Coral Protocol) for user: {user_id}")
    log_to_database("info", f"X Reply Agent (Coral Protocol) starting for user: {user_id}")
    ...
    async with MultiServerMCPClient(
        ...
        if not wait_for_mentions_tool:
            ...
            log_to_database("error", "wait_for_mentions tool not found in coral tools")
            return
        ...
        logger.info("Starting X Reply Agent (Coral Protocol) execution with optimized API usage")
        log_to_database("info", "Starting X Reply Agent (Coral Protocol) execution with optimized API usage")
