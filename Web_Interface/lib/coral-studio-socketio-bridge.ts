...
interface MCPSession {
  ...
  agentId: string;
  userId: string;
  mcpClient?: any; // Will be MultiServerMCPClient when available
}

const activeMCPSessions = new Map<string, MCPSession>();

export default function injectSocketIO(io: SocketIOServer) {
  ...
  const ns = io.of((name: string, auth: any, next: (err: Error | null, success: boolean) => void) => {
    next(null, true);
  });
...
function handleMCPIntegration(socket: Socket, ns: any) {
...
async function createMCPConnection(session: MCPSession): Promise<void> {
...
async function sendMCPMessage(session: MCPSession, data: any): Promise<void> {
...
async function listMCPAgents(session: MCPSession): Promise<any[]> {
...
function cleanupMCPSession(socketId: string): void {
...
export function getSocketSecret(): string {
  return socketSecret;
}
